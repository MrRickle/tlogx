...
device_info = namedtuple("device_info",[ "device_file", "device_id", "friendly_name", "temp_check_interval", "temp_delta_minimum", "log_maximum_interval", "last_log", "last_checks"] )
...
def do_logging(device_info):
        temperature = get_temp(device_info.temp_check_interval)
        if temperature == None:
...
def main():
    ...
    for device_file in devicefiles:
        ...
        t = perpetualTimer(deviceinfo.temp_check_interval,do_logging,deviceinfo)
        loggers.append(t)
    for t in loggers:
        t.start()
